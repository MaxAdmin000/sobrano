...
- [ ] **A6.** Зоны и тарифы доставки: 6 зон захардкожены в delivery.html — перевести в админку. `(2)`
...
- [ ] **F33.** База клиентов: авто-сборка из заказов (ФИО, телефон, email, адреса, история, сумма всего, средний чек, дата последнего заказа). `(3)`
- [ ] **F34.** Карточка клиента: история + примечания + теги. `(2)`
- [ ] **F35.** Сегменты: «новые», «постоянные», «спящие 30+ дней». `(2)`

---

## G. Аналитика и отчётность

- [ ] **G36.** Дашборд: графики заказов/выручки/конверсии/среднего чека/топ боксов на главной странице админки. `(3)`
- [ ] **G37.** Воронка: просмотры → корзина → чекаут → оплата (нужен счётчик событий). `(2)`
...
- [~] **H39.** Реф-программа: частично закрыта (2026-05-12). **Сделано:** учёт `?ref=CODE`/`?promo=CODE` в URL — `assets/cart.js · captureUrlPromoRef` сохраняет код в `localStorage[sobrano_pending_promo_v1]`, чистит URL через `history.replaceState`, при первой же загрузке каталога автоматически применяет промо и показывает toast. **Серверная валидация промокода** (`backend/src/admin.js · validatePromoServer`) при `POST /api/orders` пересчитывает `discount`/`total` от своего справочника — закрыта дыра, где клиент мог прислать любой `total`. Проверяются: `not-found`, `inactive`, `expired`, `limit-reached` (по `maxUses`/`usedCount`), `min-subtotal`. На ответе появляется `promoRejected: <причина>` для UX-сигналов. **`usedCount` декрементится при отмене заказа** (cancel) и возвращается при re-activate (cancel→paid и т.п.), флаг `promoUsageDecremented` защищает от двойного учёта.<br>**Осталось:** автогенерация персонального реф-промокода после первой оплаты (триггер при `status:paid` с `customer.email` без существующих оплат — создаём `REF-NAME-XXXX` и шлём в email-уведомлении); привязка `bonusForOwner` к owner'у (новая коллекция `bonuses[]` или `customers[].wallet`); правило «only-first-order» (отказать при `customer.email` уже встречается в `orders`); анти-self (отказ когда `customer.email === promo.refOwner.email`); фильтрация колонки/значка «реф» в админ-таблице промокодов; CMS-биндинги mechanics на `referral.html` (`friendDiscountPct`, `ownerBonusAmount` сейчас не реактивны). Требует CRM из F33. `(оценка остатка: 3)`
