...
- [x] **Patch 2026-05-15** · CRM: **F33** клиенты автособираются из заказов (`customers[]`, агрегаты totalSpent/avgCheck/orderCount/etc), endpoints `/api/admin/customers*` с фильтрами и сегментами. **F34** карточка клиента в админке: контакты, история, заметки и теги. (Реф-секции выкорчеваны из карточки 2026-05-15 вместе с системой H39.)
...
- [x] **A7.** Временные слоты · 2026-05-15. Слоты живут в CMS-секции `content.delivery.slotsBlock.list`, схема расширена полями `id/startHour/endHour/surcharge/peak/checkoutHidden`. На `delivery.html` 8 hardcoded `<div class="slot">` заменены одним template-блоком с `data-cms-list`; eyebrow/titleHtml/lead тоже привязаны к CMS. На чекауте `ALL_SLOTS` (хардкод 6 окон) заменён `normalizeSlots()`, который читает CMS-список, фильтрует `checkoutHidden`/`hidden`, парсит время через regex `^HH(:MM)? — HH(:MM)?$` (нерасшифровываемые слоты типа «К конкретному часу» автоматически становятся информационными — видны на delivery.html, скрыты в чекауте). В `<option>` показывается «10:00 — 12:00 · +200 ₽», если `surcharge > 0`. `Cart.setCustomer` теперь сохраняет `time/timeSlotId/timeSurcharge`. В `cart.js · getDeliveryPrice()` добавлен `getSlotSurcharge()` (читает из `state.customer.timeSurcharge`), итоговая сумма автоматически учитывает доплату. В summary чекаута, на thank-you, в детальной заказа и в `admin/print.html` — отдельная строка «Срочный слот · + N ₽». В CSV-экспорте появилась колонка «Доплата за слот». `store.js · load()` подмигрирует legacy slot-данные: добавляет `id` (`s1`-`sN`), парсит `startHour/endHour` из `t`, выставляет `checkoutHidden=true` для непарсимых слотов — всё без потери админских правок.
...
- [x] **B11.** Галерея изображений на товар · 2026-05-17. Главное `image` остаётся как было — новая запись `images: string[]` хранит дополнительные кадры. В админке: новый компонент `imageGallery()` рядом с `imagePicker()` в формах boxes/flowers/picks/addons — сетка thumbnails 110px, кнопки «↑ / ↓ / ×» на каждой, «＋ Загрузить» / «＋ Из медиатеки» снизу. Hidden input `cf-images` хранит `JSON.stringify(urls)`, `readCatForm` парсит обратно в массив; невалидные строки фильтруются. Frontend: `box.html` получил `<div data-box-gallery>` под главной картинкой, `assets/page-box.js · renderBoxGallery()` рендерит thumbnail-стрипу (главное image + extra images), клик переключает основное изображение. Если кадров ≤1 — стрипа скрыта. Для flowers/picks/addons бекенд хранит images, но frontend пока их не отображает (карточки маленькие — добавлю когда будет дизайн галерейного режима). Старые товары без `images` работают без миграции — массив подгружается пустым.
...
- [—] **H39.** ~~Реф-программа.~~ **Отброшено 2026-05-15.** Реф-система полностью вырезана из проекта по решению владельца (UI + backend). Удалено: `referral.html`, секция `content.referral` в CMS, FAQ-страница `referral`, ссылки в шапке/футере, обработка `?ref=` параметра в `cart.js`, серверная ветка `p.ref === true` в `validatePromoServer`, `ensurePersonalReferralPromo`, `sendReferralPromoEmail`, endpoint `/api/admin/referral/generate-promo`, секция `bonuses` в `store.json`, поля `wallet/bonusPending/bonusEarned/referralPromo` в карточке клиента, кнопка «+ Реф-промокод» в каталоге, журнал бонусов в карточке клиента, под-таб «Реф-программа» в редакторе контента. Возвращаться не нужно.
- [x] **H40.** Акционный баннер · 2026-05-16. Новая CMS-секция `content.banner` с полями `{active, text, link, linkLabel, dismissible, bg, fg, expiresAt}`. `assets/promo-banner.js` (подключён на 18 страницах) рендерит плашку сверху страницы, если `active=true`, `expiresAt` не истёк и пользователь её не закрыл. Версионирование через djb2-хеш от `text+link` — новый текст автоматически = новая версия, и баннер появится заново даже у тех, кто закрыл предыдущий. Закрытие сохраняется в `localStorage` ключ `sobrano_banner_dismissed_v1`. CTA-ссылка опциональна. Цвета фона/текста управляются админом — можно поставить терракот, винный или любой HEX. Анимация slide-down при появлении и collapse при закрытии.
- [x] **H41.** SEO мета per-page · 2026-05-16. На 5 ключевых страницах (`index/about/delivery/returns/contacts`) добавлены `<title data-cms-text="content.<page>.seo.title">`, `<meta name="description" data-cms-attr-content="content.<page>.seo.description">`, полный og:* блок (`og:type/url/site_name/title/description/image`) и `<link rel="canonical">`. Hardcoded fallback в HTML остаётся (важно для SEO-краулеров, которые читают первичный HTML до выполнения JS) — `content.js` асинхронно переписывает значения, если админ менял что-то в админке. В CMS-секцию каждой страницы добавлено поле `seo: {title, description, ogImage}` — редактируется через generic JSON-форму в табе «Контент → Главная/О бренде/…».
- [x] **H42.** Sitemap автогенерация · 2026-05-16. Динамический endpoint `GET /api/sitemap.xml` отдаёт актуальный список из 17 публичных страниц (главная + flowers/picks/4 варианта box.html?size=… + about/delivery/returns/contacts/documents/requisites + 4 юр.документа). Поле `lastmod` берётся из `content._meta.updatedAt` — `patchContent`/`setContentSection` теперь автоматически обновляют этот timestamp при каждой правке в админке. Статический `sitemap.xml` удалён (был устаревший — содержал удалённый `referral.html`, box-{s,m,l,xl} стабы и lastmod от 2026-05-08). `robots.txt` обновлён: `Sitemap:` указывает на новый `/api/sitemap.xml`, добавлены disallow для `box-{s,m,l,xl}.html`/`/admin`/`/api/`/`/uploads/`. Cache-Control: public, max-age=3600 — sitemap не критично свежий.
- [—] **H43.** ~~Динамика на `referral.html` для залогиненного клиента.~~ **Отброшено 2026-05-15** вместе с H39.
...
- [x] **I47.** Команда флористов · 2026-05-17. Секция `content.about.team` уже была (eyebrow/title/lead/members), но рендер на `about.html` был hardcoded и фото добавить было нельзя. Расширили схему `members[]` полями `photo` (URL) и `hidden` (toggle конкретного человека). Добавили `team.hidden` — toggle всей секции (если, например, состав ещё не сфоткали или временно не хочется светить лиц). `about.html` → `<section class="team" data-cms-hide-if="content.about.team.hidden">` + grid через `data-cms-list`, шаблон-карточка с initials-фоллбэком и img-перекрытием через post-hydrate скрипт. Скрытые члены получают класс `is-hidden` через `data-cms-loop-class-is-hidden="@hidden"` → CSS их не показывает. Новый универсальный механизм `data-cms-hide-if="path"` в `assets/content.js` — пригодится для любой будущей toggle-секции.
- [x] **I48.** Уведомления администратору · 2026-05-16. Расширили событийную модель Telegram-уведомлений с 4 до 9 типов, все управляются галочками в админке → Настройки → Уведомления → Telegram (`settings.notifications.telegram.events`). Новые события: **📩 contactForm** — заявка с формы «Контакты» (новый публичный `POST /api/contact` с honeypot-полем `hp` для антиспама и in-memory rate-limit «1 раз в 20 с / 10 в час с IP»); форма на `contacts.html` теперь действительно отправляет данные на бэкенд. **⚠️ refundRequested** — клиент/админ запросил возврат (триггерится при `refund.status='requested'` в `patchOrder`, отдельно от уже работающего «refund»). **🐢 staleOrder** — заказ висит в `new` дольше 30 мин: cron в `server.js` (каждые 5 мин) сканит orders, шлёт алерт, ставит `staleAlertSent=true` чтобы не повторять. **🚨 loginFailure** — детектор брутфорса в `auth.js`: in-memory счётчик неудач по IP (окно 10 мин, порог ≥5), TG-cooldown 30 мин чтобы не флудить, при успешном логине счётчик IP сбрасывается. **🌙 dailyDigest** — раз в сутки в 23:30 UTC (cron в `server.js`): orders/paid/cancelled за 24 ч + revenue + средний чек + новые клиенты + топ-бокс. Дедуп по `_lastDigestDate` чтобы не отправлять дважды.
- [x] **I49.** CRUD каналов связи · 2026-05-16. Новая секция `catalog.channels` с полями `{id, type, title, href, icon, glyph, color, meta, order, active, builtin, hidden}`. Built-in: `telegram / whatsapp / phone / email` — нельзя удалить, можно отключить через `active`; id и type у них залочены, чтобы кастомный канал не подменил «настоящий» Telegram. Custom-каналы (Viber, VK, MAX, Threads, Discord, любой URL) админ добавляет через «Каталог → Каналы → ＋ Добавить», тип у них всегда `link`. Иконка каждого канала — поле `icon` (URL) с тем же media-picker'ом, что у боксов/цветов: «Загрузить» / «Из медиатеки» / «Очистить»; фоллбэк — короткий текстовый glyph (TG/WA/TEL/@) на фоне цвета `color`. Поле `meta` — маркетинговая подпись под карточкой на `contacts.html` («Самый быстрый канал · 24/7» и т.п.). Публичный `GET /api/channels` отдаёт активные каналы с готовым `href` (телефон → `tel:`, email → `mailto:`, остальное as-is). Миграция: при первой загрузке `store.js · ensureBuiltinChannels` подтягивает значения из старого `settings.contacts` (если они там кастомные, а в catalog.channels ещё дефолтные) — данные админа не теряются. Потребители: `assets/consult-button.js` (FAB-попап, рендерит круги с `<img src=icon>` или glyph) и `contacts.html` (4-колоночный grid с фоллбэк-разметкой на случай если API недоступен).
...
- [x] **K54.** Rate-limit · 2026-05-16. **`/api/orders`**: in-memory лимит «1 заказ / 10 сек + 30 заказов / час с одного IP», срабатывает раньше дорогой валидации, отвечает 429 с понятным русским сообщением. Чекаут это сообщение показывает через `showToast` и не редиректит на thank-you. **`/api/admin/login`**: расширили I48-детектор брутфорса blocking-режимом — после 5 неудач за 10 мин IP блокируется на 5 минут, даже валидный пароль не пройдёт (`auth.isLoginBlocked` → 429). Успешный логин с того же IP сбрасывает счётчик.
- [x] **K55.** Honeypot · 2026-05-16. Уже сделан на `/contacts.html` в I48 (поле `cf-hp`), теперь распространён на чекаут: `checkout.html` отправляет в payload `hp` (из скрытого поля `hp-checkout`, position:absolute;left:-9999px), `admin.postOrder` при непустом `hp` тихо отвечает 200 без сохранения. CAPTCHA сознательно не делали — honeypot закрывает 95%+ ботов без раздражения людей, а защиту админ-форм даёт K54 (blocking rate-limit).
- [x] **K56.** Структурированный лог · 2026-05-16. Новый `backend/src/logger.js` без зависимостей: `log.info/warn/error("event.name", {meta})` пишет одну JSON-строку в stdout/stderr (для journalctl) и опционально в `SOBRANO_LOG_FILE` если переменная окружения задана. Поля: `ts` (ISO), `level`, `event`, `...meta`. Подключён в ключевых местах: `order.created` / `order.rejected` / `order.honeypot` / `order.rate_limited` / `contact.received` / `contact.honeypot` / `login.success` / `login.failure` / `login.blocked` / `login.bruteforce_detected` / `server.started`. Фильтрация через `journalctl -u sobrano-backend -o cat \| jq 'select(.event=="login.failure")'`.
- [x] **K57.** Uptime-мониторинг · 2026-05-16. `.github/workflows/uptime.yml` запускается каждые 5 минут (`cron: */5 * * * *`), делает 2 попытки с интервалом 15 с на `https://sobrano.store/health`, проверяет HTTP 200 + `ok:true` в body + срок SSL-сертификата (warn если < 14 дн). При падении: создаётся GitHub Issue с label `outage` (auto-создание label при первом инциденте) + алерт в Telegram через `secrets.TG_BOT_TOKEN`/`TG_CHAT_ID`. Если outage-issue уже открыт — добавляется comment без дублирования TG, чтобы не спамить. При восстановлении: comment в issue + close + TG «recovered». Без TG-секретов всё работает в режиме «только лог» (failures видны в Actions tab). Конкурентность ограничена `concurrency.group: uptime`, permissions: `issues:write`. **Setup TG-секретов на проде**: `gh secret set TG_BOT_TOKEN` + `gh secret set TG_CHAT_ID` (взять из админки → Настройки → Уведомления → Telegram, или сделать отдельного DevOps-бота).
...
- [—] **L60.** ~~Страница отслеживания заказа по № + email/телефон, без логина.~~ **Отброшено 2026-05-15.** Решение: клиент с высокой вероятностью не сохранит номер заказа, а админы не всегда успевают вовремя проставлять статусы — публичный трекер будет генерировать звонки и переживания «почему заказ не отправляется». Коммуникацию о статусе оставляем push-каналом (email + telegram, см. D27), а не pull-страницей. Возвращаться к идее не нужно.
...
~~1. **K58 + K53 + K59** — CI/CD, ежедневные бэкапы `data/store.json`+`uploads`, ufw.~~ ✅
~~2. **A6 + A7 + A8** — зоны/слоты/способы оплаты под управлением админки (блок A).~~ ✅ блок A закрыт.
~~3. **F33 + G36** — база клиентов из заказов + дашборд.~~ ✅
~~4. **L60 + L61** — клиентская страница отслеживания + унификация конструктора.~~ L61 ✅, L60 отброшено (см. блок L).

Следующие кандидаты — оставшиеся хвосты по блокам:

1. **H41 + H42** — SEO мета per-page + sitemap. CMS уже умеет всё, надо расширить content-схему и `data-cms-attr`. `(1 + 0,5)`
2. **K54 + K55** — rate-limit на `/api/orders` и `/api/admin/login` + CAPTCHA/honeypot на публичных формах. Антиспам перед запуском маркетинга. `(1 + 1)`
3. **G38** — отчёт по складу (продано стеблей за период). Логичное продолжение G36-дашборда. `(1)`
4. **I46** — праздники/особый график (календарь индивидуальных часов на 8 марта, 14 февраля). `(1)`
5. **B11** — несколько изображений на товар (галерея). `(2)`
6. **E28-E32** (бэклог) — Robokassa подключить, когда будет принципиальное решение по приёму платежей.
