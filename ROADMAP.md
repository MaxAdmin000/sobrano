...
- [ ] **I49.** CRUD каналов связи. По аналогии с A8 (способы оплаты): новая секция `catalog.channels` с полями `{id, type, title, href, icon, order, active, builtin}`. Built-in: `telegram / whatsapp / phone / email` (нельзя удалить, можно отключить через `active`). Custom-методы (Viber / VK / MAX / Threads / Discord / форма WebApp / любой URL) админ добавляет через «Каталог → Каналы → ＋ Добавить». **Иконка каждого канала** — поле `icon` (URL изображения) с тем же media-picker'ом, что у боксов/цветов: кнопки «Загрузить» / «Из медиатеки» / «Очистить»; фоллбэк на короткий текстовый glyph (TG/WA/TEL/@), если иконка пустая. Публичный `GET /api/channels` фильтрует по `active`. Сейчас зашитый список из 4 ключей в `settings.contacts` мигрируется в новую секцию `catalog.channels` при первом запуске. Frontend-потребители: `assets/consult-button.js` (FAB-попап на сайте, рендерит круги с icon вместо текстовых глифов) и `contacts.html` (карточки каналов в hero/`channels`-секции CMS). `(3)`
